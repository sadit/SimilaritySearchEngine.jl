...
`SimilaritySearchServer` exposes [`SimilaritySearchEngine`](../index.qmd) — the embedded
engine documented in the rest of this site — through a REST API, two CLIs, and an asynchronous job
...
[architecture guide](../architecture.qmd), not here. The two packages live in one
repository and are documented as one thing on purpose: the seam between them is the engine's
public API, and both sides of that seam are described here.
...
- The [server tutorial](tutorial.qmd) — the same surface from the outside: a dataset created,
  indexed, queried, and a heavy job run, in eight steps.
- The [engine's architecture](../architecture.qmd) — the storage layout, the engine hierarchy,
  the concurrency model and the typed errors this package maps.
